...
const TakeSPL: FC = () => {
    ...
    const onClick = useCallback(async () => {
        ...
        let holder = Keypair.fromSecretKey(DEMO_WALLET);
        ...
        try {
            var fromTokenAccount = await token.getOrCreateAssociatedAccountInfo(holder.publicKey);
            var toTokenAccount = await token.getOrCreateAssociatedAccountInfo(publicKey);
    ...
    return (
        <Button variant="contained" color="secondary" onClick={onClick} disabled={!publicKey}>
            Take Custom SPL token/NFT (devnet)
        </Button>
    );
};

export default TakeSPL;